...
use crate::{
    config::{CLOCK_FREQ, MAX_SYSCALL_NUM}, fs::{open_file, OpenFlags}, mm::{translated_byte_buffer, translated_refmut, translated_str, MapPermission}, task::{
...
pub fn sys_spawn(_path: *const u8) -> isize {
    ...
    let app_file = open_file(path.as_str(), OpenFlags::RDONLY);
    if let Some(file) = app_file {
...
pub fn sys_set_priority(_prio: isize) -> isize {
    trace!(
        "kernel:pid[{}] sys_set_priority set priority to {}",
        current_task().unwrap().pid.0,
        _prio
    );
    if _prio > 1 {
